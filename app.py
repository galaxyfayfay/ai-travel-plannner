"""
app.py v10 — AI Travel Planner Pro
修复：LANG 在文件顶部从 session_state 初始化，
      sidebar 之前所有 _t() 调用均可正常工作。
"""
...
try:
    ...
    _FB = {
        ...
        "pick_country":"🌐 Country / Region","pick_city":"🏙️ City",
        "city_override":"✏️ Or type any city:","city_placeholder":"e.g. Kyoto, Paris…",
        "hotel_label":"🏨 Where are you staying?","hotel_placeholder":"Hotel name or address",
        "depart_label":"🚩 Day 1 start","depart_placeholder":"e.g. Tokyo Station",
        "arrive_label":"🏁 Final departure","arrive_placeholder":"e.g. Narita Airport",
        "plan_heading":"🗓️ Plan your days","how_many_days":"How many days?",
        "what_todo":"What do you want to do?","day_prefs_heading":"Day-by-day preferences",
        "day_prefs_caption":"Choose area, rating, and place count per day.",
        "area_label":"Area","min_rating_label":"Min rating",
        "daily_budget_label":"💰 Daily budget (USD)","all_area_label":"Area (all days)",
        "build_btn":"🚀 Build my itinerary","refresh_btn":"🔄 Try different places",
        "loading_districts":"Loading districts…","loading_neighbourhoods":"Loading neighbourhoods…",
        "finding_dest":"Finding destination…","looking_up_locations":"Looking up locations…",
        "finding_places":"Finding places in","building_itin":"Building itinerary…",
        "metric_places":"📍 Places","metric_days":"📅 Days","metric_stops":"🗓️ Stops",
        "metric_rating":"⭐ Avg Rating","metric_budget":"💰 Budget",
        "tbl_seq":"#","tbl_day_stop":"Day/Stop","tbl_time":"Time","tbl_place":"Place",
        "tbl_district":"District","tbl_type":"Type","tbl_rating":"Rating",
        "tbl_transport":"Getting There","tbl_contact":"Contact",
        "map_heading":"🗺️ Route Map","map_caption":"Click markers for details",
        "budget_heading":"💰 Cost Estimate","budget_over":"🔴 Some days exceed budget.",
        "budget_total":"Total","budget_breakdown":"📊 Breakdown",
        "export_heading":"📤 Save / Share","export_download":"📄 Download HTML",
        "export_download_btn":"⬇️ Download","export_calendar":"📅 Google Calendar",
        "export_date":"Start date?","export_caption":"Open in browser → Ctrl+P → Save as PDF",
        "rec_heading":"💡 More Places","rec_caption":"Worth exploring.",
        "rec_refresh":"🔄",
        ...
        "wishlist_heading":"💫 Wishlist","wishlist_empty":"Wishlist empty.",
        "wishlist_add":"❤️ Save","wishlist_saved":"✅ Saved!",
        "points_heading":"🎫 Points","points_checkin":"📍 Check In",
        "points_checkin_done":"✅ Checked in!","points_redeem":"🎁 Redeem",
        "points_balance":"Balance","points_history":"History",
        "auth_login":"🔐 Sign In","auth_register":"📝 Register","auth_logout":"🚪 Sign Out",
        "auth_username":"Username","auth_password":"Password","auth_email":"Email",
        "auth_login_required":"Please sign in.",
        "collab_heading":"🤝 Collaborate","collab_share_link":"📋 Share code",
        "mustsee_heading":"⭐ Must-See","mustsee_caption":"Curated highlights",
        "err_city_not_found":"City '{city}' not found.",
        "err_itinerary_failed":"Itinerary error: {err}",
        "err_map_failed":"Map error: {err}","err_no_places":"No places found.",
        "err_export_failed":"Export error: {err}",
        "data_source":"📡 Data","data_radius":"Within 8 km",
        "last_stop":"Last stop","transport_comparison":"🚇 Transport Options",
        "swap_heading":"🔄 Swap a Stop","swap_caption":"Replace with alternative.",
...
try:
    from transport_planner import (estimate_travel, estimate_all_modes,
...
try:
    from wishlist_manager import (add_to_wishlist, remove_from_wishlist, get_wishlist,
...
COST_FL= {"🏛️ Attraction":4,"🍜 Restaurant":6,"☕ Café":3,
           "🌿 Park":0,"🛍️ Shopping":8,"🍺 Bar/Nightlife":5,"🏨 Hotel":0,"Transport":1}
...
def _amap_geocode(addr):
    try:
        ...
        if str(r.get("status"))=="1" and r.get("geocodes"):
            ...
            if "," in loc: lon,lat=map(float,loc.split(",")); return lat,lon
    except Exception: pass
    return None
...
def _nominatim(q):
    try:
        r=requests.get("https://nominatim.openstreetmap.org/search",
            ...
            headers={"User-Agent":"TravelPlannerPro/10"},timeout=9).json()
...
def _get_nominatim_districts(city):
    ...
    try:
        r=requests.get("https://nominatim.openstreetmap.org/search",
            ...
            headers={"User-Agent":"TravelPlannerPro/10"},timeout=8).json()
        if not r: return []
        ...
        for url in ["https://overpass-api.de/api/interpreter","https://overpass.kumi.systems/api/interpreter"]:
            try: els=requests.post(url,data={"data":q},timeout=18).json().get("elements",[]); break
...
def _parse_amap(pois,cn_kw,tl,limit,seen):
    ...
    for p in pois:
        ...
        out.append({"name":nm,"lat":plat,"lon":plon,"rating":rating,
                    ...
                    "type":cn_kw,"type_label":tl,"district":p.get("adname") or "",
                    "description":tdesc(cn_kw)})
    return out
...
def _amap_by_adcode(adcode,cn_kw,tl,limit):
    ...
    def _f(kw,tc,pg):
        ...
        if tc: p["types"]=tc
        return requests.get("https://restapi.amap.com/v3/place/text",params=p,timeout=10).json()
    for pg in range(1,6):
...
def _amap_around(lat,lon,cn_kw,tl,limit,radius=8000):
    ...
    for kw in [cn_kw]+AMAP_ALT.get(tl,[]):
        ...
        for pg in range(1,5):
            if len(places)>=limit: break
            try:
                p={"key":AMAP_KEY,"location":f"{lon},{lat}","radius":radius,
...
def search_cn(lat,lon,tls,lpt,adcode="",dname=""):
    ...
    for tl in tls:
        cn=PTYPES[tl]["cn"]
        ps,e=(_amap_by_adcode(adcode,cn,tl,lpt) if adcode else _amap_around(lat,lon,cn,tl,lpt))
...
def demo_places(lat,lon,tls,n,seed,district=""):
    random.seed(seed)
    NAMES={
        "🏛️ Attraction":["Grand Museum","Sky Tower","Ancient Temple","Art Gallery","Historic Castle",
                         "Night Market","Cultural Center","Scenic Viewpoint","Old Town Square"],
        ...
        "🌿 Park":["Riverside Park","Sakura Garden","Central Park","Bamboo Grove","Zen Garden"],
    ...
    for tl in tls:
        ...
        for i,nm in enumerate(nms[:n]):
            ...
            out.append({"name":nm,"lat":round(clat+random.uniform(-.006,.006),5),
                        ...
                        "rating":round(random.uniform(4.,4.9),1),
                        "address":"Preview — connect to internet for real data",
                        "phone":"","website":"","type":PTYPES[tl]["cn"] if tl in PTYPES else tl,
...
def fetch_all_places(clat,clon,country,is_cn,tls_t,lpt,
    ...
    if not out:
        ...
        if is_cn:
            warn=("⚠️ 高德 API 无数据（检查Key IP白名单设置），" if api_errs
...
def render_auth_sidebar():
    ...
    if user:
        ...
        with t2:
            ...
            re=st.text_input(_t("auth_email"),key="re_e")
        ...
        for di,(dl,stops) in enumerate(itinerary.items()):
            ...
            for si in range(len(stops)-1):
                ...
                folium.PolyLine([[a["lat"],a["lon"]],[b["lat"],b["lon"]]],
                    color=dc,weight=3.8,opacity=.75,dash_array="7 4").add_to(m)
                mid=[(a["lat"]+b["lat"])/2,(a["lon"]+b["lon"])/2]
                pt=(f"<div style='font-size:.8rem'><b>{dl}·Leg {si+1}</b><br>"
                    f"{tr.get('mode','—')}<br>⏱ {tr.get('duration','—')} · "
                    f"📏 {tr.get('distance_km','—')} km<br>"
                    f"{a['name']} → {b['name']}</div>")
                folium.Marker(mid,popup=folium.Popup(pt,max_width=210),
                    tooltip="🚦 Travel",
                    icon=folium.DivIcon(
                        html=f'<div style="width:9px;height:9px;border-radius:50%;background:{dc};border:2px solid white"></div>',
                        icon_size=(9,9),icon_anchor=(4,4))).add_to(m)
    for _,row in df.iterrows():
        v=vi.get(row["name"])
        if v: di,sn,_=v; color=DAY_COLORS[di%len(DAY_COLORS)]; label=str(sn); day_info=f"Day {di+1}·Stop {sn}"
        else: color="#b0a090"; label="·"; day_info="Not scheduled"
        addr=str(row.get("address",""))
        ph=str(row.get("phone",""))
        pop=(f"<div style='min-width:180px'><div style='font-weight:700'>{row['name']}</div>"
        ...
        folium.Marker([row["lat"],row["lon"]],
            popup=folium.Popup(pop,max_width=250),tooltip=f"{day_info}—{row['name']}",
            icon=folium.DivIcon(
                html=(f'<div style="width:22px;height:22px;border-radius:50%;background:{color};'
                      ...
                      f'color:white;font-size:11px;font-weight:700;box-shadow:0 2px 6px rgba(0,0,0,.3)">{label}</div>'),
                icon_size=(22,22),icon_anchor=(11,11))).add_to(m)
    def sm(c,ic,tip):
        folium.Marker(list(c),tooltip=tip,
            icon=folium.DivIcon(html=f'<div style="font-size:22px">{ic}</div>',
...
def render_table(df,itinerary,day_budgets,country,city=""):
    ...
    if itinerary:
        for di,(dl,stops) in enumerate(itinerary.items()):
            for si,s in enumerate(stops): stop_map[s["name"]]=(di,si+1,s)
    n2r={row["name"]:row for _,row in df.iterrows()}
    scheduled=[]
    if itinerary:
        for di,(dl,stops) in enumerate(itinerary.items()):
            for si,s in enumerate(stops):
                if s["name"] in n2r: scheduled.append((di,si+1,s["name"]))
    snames={x[2] for x in scheduled}
    unscheduled=[row for _,row in df.iterrows() if row["name"] not in snames]

    rows_html=""; seq=0; cur_day=-1
    for di,sn,nm in scheduled:
        seq+=1; row=n2r[nm]; color=DAY_COLORS[di%len(DAY_COLORS)]
        d_usd=day_budgets[di] if di<len(day_budgets) else day_budgets[-1]
        if di!=cur_day:
            cur_day=di
            ds=list((itinerary or {}).get(f"Day {di+1}",[]))
            em,lb,_,__=budget_level(d_usd)
            rows_html+=(f'<tr class="day-hdr"><td colspan="9">'
                        f'Day {di+1}  ·  {len(ds)} stops'
                        f'  <span class="budget-pill">{em} ${d_usd}/day</span>'
                        f'</td></tr>')
        _,_,sd=stop_map[nm]
        rows_html+=_trow(seq,row,di+1,sn,sd,color,d_usd,country)

    st.markdown(
        '<div style="overflow-x:auto"><table class="itin-table"><thead><tr>'
        f'<th>{_t("tbl_seq")}</th><th>{_t("tbl_day_stop")}</th>'
        f'<th>{_t("tbl_time")}</th><th>{_t("tbl_place")}</th>'
        f'<th>{_t("tbl_district")}</th><th>{_t("tbl_type")}</th>'
        f'<th>{_t("tbl_rating")}</th><th>{_t("tbl_transport")}</th>'
        f'<th>{_t("tbl_contact")}</th>'
    ...
    # Must-see
    if DATA_MGR_OK:
        ms=render_must_see_panel(city,LANG)
        if ms: st.markdown(ms,unsafe_allow_html=True)
...
def _trow(seq,row,day_n,stop_n,sd,color,daily_usd,country):
    ...
    dc=(f'<span style="background:{color};color:#fff;border-radius:50%;width:22px;height:22px;'
        f'display:inline-flex;align-items:center;justify-content:center;font-weight:700;font-size:11px">'
        f'{stop_n}</span><br><small style="color:#999">D{day_n}</small>')
    tc=sd.get("time_slot","—") if sd else "—"
    tr=sd.get("transport_to_next") if sd else None
    if tr:
        dk=tr.get("distance_km",0) or 0
        _,tc2=transport_cost_estimate(dk,daily_usd,country)
        ti=tr.get("transit_info",""); ti_h=f'<br><small style="color:#999">{ti}</small>' if ti else ""
        rc=(f'{tr["mode"]}<br><small style="color:#888">⏱ {tr["duration"]} · {dk} km<br>'
            f'💸 {tr.get("cost_str",tc2)}{ti_h}</small>')
    else:
        et=sd.get("end_transport") if sd else None
        rc=(f'{et["mode"]}<br><small style="color:#888">→ {et.get("to_label","End")}</small>'
            if et else f'<small style="color:#bbb">{_t("last_stop")}</small>')
    tl=row.get("type_label","") or row.get("type","")
    rstr=f'⭐ {row.get("rating",0):.1f}' if row.get("rating",0)>0 else "—"
    desc=row.get("description","")
    cost_h=""
    if daily_usd>0 and tl:
        _,cs=cost_estimate(tl,daily_usd,country)
        cost_h=f'<div style="font-size:.75rem;color:#c97d35">💰 {cs}</div>'
    ...
    if ph: ct+=f'<div style="font-size:.75rem">📞 {ph}</div>'
    if not ct: ct="—"
    return (f'<tr style="background:{bg}"><td style="text-align:center;color:#999">{seq}</td>'
            f'<td style="text-align:center">{dc}</td>'
            f'<td style="color:#777;font-size:.83rem;white-space:nowrap">{tc}</td>'
            f'<td>{nc}</td><td style="color:#777;font-size:.8rem">{row.get("district","—")}</td>'
            f'<td style="font-size:.8rem">{tl}</td><td>{rstr}</td>'
            f'<td style="font-size:.8rem">{rc}</td><td style="font-size:.75rem">{ct}</td></tr>')
...
def _render_extra(unscheduled,day_budgets,country):
    ...
    for cn,tls in CATS:
        items=[]; [items.extend(by_type.get(tl,[])) for tl in tls]; [covered.add(t) for t in tls]
    ...
    for cn,places in cat_data:
        ...
        with c1: st.markdown(f'<div style="font-weight:600;margin:10px 0 4px">{cn} <span style="color:#bbb;font-size:.78rem">({min(10,len(places))}/{len(places)})</span></div>',unsafe_allow_html=True)
        with c2:
            if st.button(_t("rec_refresh"),key=f"_rf_{cn}"): st.session_state[sk]=(st.session_state[sk]+1)%9999
        import random as _r; _r.seed(st.session_state[sk])
        picks=sorted(_r.sample(places,min(10,len(places))),key=lambda r:r.get("rating",0),reverse=True)
        cards=""
        for p in picks:
            nm=str(p.get("name","")); tl=str(p.get("type_label","") or p.get("type",""))
            rat=p.get("rating",0); dist=str(p.get("district","") or "—")
            addr=str(p.get("address","") or "")[:55]; ph=str(p.get("phone","") or "")
            _,cs=cost_estimate(tl,avg,country)
            addr_h=f'<div style="font-size:.74rem;color:#999">📍 {addr}</div>' if addr and "demo" not in addr.lower() else ""
            ph_h=f'<div style="font-size:.74rem;color:#999">📞 {ph}</div>' if ph else ""
            cards+=(f'<div class="rec-card"><div class="rc-name">{nm}</div>'
                    f'<div class="rc-meta">{tl} · {dist}</div>{addr_h}{ph_h}'
...
def render_budget_summary(itinerary,day_budgets,country,days):
    ...
    for di,(dl,stops) in enumerate(itinerary.items()):
        if not stops: continue
        d_usd=day_budgets[di] if di<len(day_budgets) else day_budgets[-1]
        t=sum((cost_estimate(s.get("type_label",""),d_usd,country)[0]
               +transport_cost_estimate((s.get("transport_to_next") or {}).get("distance_km",0) or 0,d_usd,country)[0])
              for s in stops)
        tots.append((dl,t,d_usd))
    if not tots: return
    ...
    for i,(dl,t,du) in enumerate(tots):
        with cols[i%(nc-1)]:
            ...
            st.markdown(f'<div class="bsum-card"><div class="day-lbl">{dl}</div>'
                        f'<div class="day-amt">${round(t)}{"🔴" if over else ""}</div>'
                        f'<div class="day-rng">{rng}</div>'
                        f'<div class="day-bud">{em} {lb} · ${du}/day</div></div>',
                        unsafe_allow_html=True)
    with cols[-1]:
        lo=round(gt*.8); hi=round(gt*1.2)
        gs=(f"${lo}–${hi}" if country=="US"
            else f"${lo}–${hi} ({sym}{round(lo*rate)}–{sym}{round(hi*rate)})")
        st.markdown(f'<div class="bsum-card" style="background:#fff7ed;border:1px solid #e8a558">'
                    f'<div class="day-lbl">{_t("budget_total")}</div>'
                    f'<div class="day-amt" style="font-size:1.5rem">${round(gt)}</div>'
                    f'<div class="day-rng">{gs}</div>'
                    f'<div class="day-bud">{days}d · ${gb} total</div></div>',
                    unsafe_allow_html=True)
    if any_over:
        st.markdown(f'<div style="background:#fff3cd;border-radius:8px;padding:10px;margin-top:8px;font-size:.85rem">{_t("budget_over")}</div>',
                    unsafe_allow_html=True)
    with st.expander(_t("budget_breakdown")):
        ...
        for di,(dl,stops) in enumerate(itinerary.items()):
            ...
            for s in stops:
                ...
                if tr:
                    dk=tr.get("distance_km",0) or 0; _,tr2=transport_cost_estimate(dk,du,country)
    ...
    with st.expander(_t("transport_comparison"),expanded=False):
        for di,(dl,stops) in enumerate(itinerary.items()):
            ...
            for si in range(len(stops)-1):
                a,b=stops[si],stops[si+1]
                st.markdown(
                    render_transport_comparison(a["lat"],a["lon"],b["lat"],b["lon"],
                        a["name"],b["name"],country=country,city=city,
                        daily_usd=du,lang=LANG),
                    unsafe_allow_html=True)

# ══════════════════════════════════════════════════════════════════
# SWAP PANEL
# ══════════════════════════════════════════════════════════════════
def render_swap_panel(itinerary,df):
    if not itinerary or not WISHLIST_OK: return
    with st.expander(_t("swap_heading"),expanded=False):
        st.caption(_t("swap_caption"))
        day_opts=list(itinerary.keys())
        sd=st.selectbox("Day",day_opts,key="sw_day")
        stops=itinerary.get(sd,[])
        if not stops: st.info("No stops."); return
        si=st.selectbox("Stop",range(len(stops)),
                        format_func=lambda i:f"Stop {i+1}: {stops[i]['name']}",key="sw_stop")
        cur=stops[si]; ctype=cur.get("type_label","")
        used={s["name"] for sl in itinerary.values() for s in sl}
        cands=df[(df["type_label"]==ctype)&(~df["name"].isin(used))].sort_values("rating",ascending=False).head(8)
        if cands.empty: st.info("No alternatives found."); return
        alt=st.selectbox("Replace with",cands["name"].tolist(),key="sw_alt")
        if st.button("✅ Confirm Swap",key="sw_ok",use_container_width=True):
            ar=cands[cands["name"]==alt].iloc[0].to_dict()
            new_it=swap_place_in_itinerary(st.session_state.get("_itin",itinerary),sd,si,ar)
            st.session_state["_itin"]=new_it
            st.success(f"✅ '{cur['name']}' → '{alt}'"); st.rerun()
...
def build_pdf(itinerary,city,day_budgets,country):
    ...
    clat=sum(mlats)/len(mlats) if mlats else 35.; clon=sum(mlons)/len(mlons) if mlons else 139.
    days_h=""
    for di,(dl,stops) in enumerate(itinerary.items()):
        if not stops: continue
        ...
        days_h+=(f"<h3 style='color:{c}'>{cl(dl)} — {len(stops)} stops · ${du}/day</h3>"
                 ...
                 f"<thead><tr style='background:#f5ede0'><th>#</th><th>Time</th><th>Place</th>"
                 f"<th>Type</th><th>District</th><th>Rating</th><th>Route</th></tr></thead>"
    ...
    html=(f'<!DOCTYPE html><html><head><meta charset="utf-8">'
          ...
          f'<p style="color:#bbb;font-size:.78rem;margin-top:32px">Generated by Trip Planner · '
          f'💡 Ctrl+P → Save as PDF</p>'
          ...
          f'[{",".join(mjs)}].forEach(function(mk){{var ic=L.divIcon({{html:'
          f'"<div style=\\"width:22px;height:22px;border-radius:50%;background:"+mk.c+'
          f'";border:2px solid white;display:flex;align-items:center;justify-content:center;'
          f'color:white;font-size:11px;font-weight:700\\">"+mk.s+"</div>",'
          f'iconSize:[22,22],iconAnchor:[11,11]}});'
          f'L.marker([mk.lat,mk.lon],{{icon:ic}}).bindPopup("<b>"+mk.n+"</b><br>Day "+mk.d+" Stop "+mk.s).addTo(m);}});'
          f'[{",".join(pjs)}].forEach(function(pl){{L.polyline(pl.pts,{{color:pl.c,weight:3.5,opacity:.75,dashArray:"7 4"}}).addTo(m);}});'
          f'</script></body></html>')
...
def render_export_panel(itinerary,city,day_budgets,country):
    ...
    with c1:
        ...
        try:
            ...
            st.download_button(_t("export_download_btn"),data=data,
        ...
        if urls:
            ...
            for dl,items in dseen.items():
                with st.expander(f"📅 {dl} ({len(items)} events)",expanded=False):
                    for it in items:
                        st.markdown(f'<a href="{it["url"]}" target="_blank" '
                                    f'style="text-decoration:none;color:#3a8fd4">'
                                    f'➕ Stop {it["stop"]}: {it["name"][:32]}</a>',
                                    unsafe_allow_html=True)

# ══════════════════════════════════════════════════════════════════
# ④  SIDEBAR  (语言选择器在这里 → 更新 session_state → 下次 rerun 生效)
# ══════════════════════════════════════════════════════════════════
with st.sidebar:
    # 语言选择器
    ...
    ndays =int(days)
    st.markdown(f'<div style="font-size:.85rem;color:#888;margin-bottom:4px">{_t("what_todo")}</div>',
                unsafe_allow_html=True)
    ...
    amap_dists=st.session_state.get(dk,[]); adcode_map={}; center_map={}
    for d in amap_dists:
    ...
    if is_cn and amap_dists:
        ...
        pdo=(["Auto (city-wide)"]+st.session_state.get(dnk,[])) or ["Auto (city-wide)"]

    st.markdown(f'<div class="sec-head">{_t("day_prefs_heading")}</div>',unsafe_allow_html=True)
    ...
    if ndays<=7:
        ...
        for di,tab in enumerate(tabs):
            with tab:
                ...
                if auto:
                    ...
                    day_adcodes.append(adcode_map.get(ds,"")); day_district_names.append(ds)
                ...
                st.markdown(f'<div class="budget-pill">${du}/day'+(f'  ≈ {lp}' if lp else '')+'</div>',
                            unsafe_allow_html=True)
        ...
        auto=(ds=="Auto (city-wide)")
        _adc="" if auto else adcode_map.get(ds,""); _dn="" if auto else ds
    ...
    if ref: st.session_state.seed=random.randint(1,99999); st.cache_data.clear(); gen=True

    # Wishlist + Points (logged-in)
    user=_cur_user()
    ...
    st.caption(f"{_t('data_source')}: {api_src}  ·  {_t('data_radius')}")

# ══════════════════════════════════════════════════════════════════
# ⑤  HERO  ← LANG 此时已由 sidebar selectbox 正确设置
# ══════════════════════════════════════════════════════════════════
st.markdown(
...
def _run_display(it,df,ci,nd,bud,ctr,tys,lat,lon,hc,dc,ac):
    ...
    except Exception as e: st.error(_t("err_map_failed",err=str(e)))
...
if gen:
    ...
    with st.spinner(f"{_t('finding_places')} {city_input.title()}…"):
        try:
            df,warn=fetch_all_places(lat,lon,country,is_cn,
                tuple(sel_types),lpt,tuple(day_adcodes),tuple(day_district_names),
                tuple(day_anchor_lats),tuple(day_anchor_lons),st.session_state.seed)
        except Exception as e: st.error(f"Search error: {e}"); st.stop()
    if warn: st.warning(warn)
    if df is None or df.empty: st.error(_t("err_no_places")); st.stop()

    itinerary={}
    if not AI_OK: st.error(f"ai_planner error: {_AI_ERR}")
    else:
        with st.spinner(_t("building_itin")):
            try:
                itinerary=generate_itinerary(
                    ...
                    country=country,city=city_input,day_budgets=day_budgets)
            except Exception as e: st.error(_t("err_itinerary_failed",err=str(e)))

    if itinerary:
        st.session_state.update({"_itin":itinerary,"_df":df,"_city":city_input,
            "_ndays":ndays,"_budgets":day_budgets,"_country":country,"_types":list(sel_types),
            "_lat":lat,"_lon":lon,"_hotel":hotel_c,"_depart":depart_c,"_arrive":arrive_c,"_lang":LANG})
        user=_cur_user()
        if user and WISHLIST_OK:
    ...
    _run_display(
        st.session_state["_itin"], st.session_state["_df"],
